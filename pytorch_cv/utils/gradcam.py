...
from .utils import Denormalize, ToNumpy
    ...
    def _cam_image(self, norm_image):
        ...
        for layer, gc in self.gradcam.items():
            ...
            cam_heatmap, cam_result = visualize_cam(
                mask,
                Denormalize(norm_image, self.mean, self.std, out_type="tensor").clone().unsqueeze_(0).to(self.device),  # type: ignore
            )
            heatmap[layer], result[layer] = ToNumpy(cam_heatmap), ToNumpy(cam_result)
        return {
